...
\textcolor{vert}{Par exemple, sur la variable «\textit{repairer\_avg\_invoice\_vs\_market} » : $(0,661 - 0,953) \times (-0,366) + (0,339 - 0,048) \times 1,965 = 0,680$.}
...
On observe sur la figure~\ref{fig:distribution-points} que les sinistres frauduleux sont principalement concentrés sur la droite du graphique, correspondant aux scores élevés, à contrario des sinistres non frauduleux qui sont concentrés à gauche.
...
Malgré sa simplicité et son interprétabilité, le modèle de scoring en points présente plusieurs limites dont il convient de tenir compte. 

\textbf{En premier lieu, le principe d'additivité et l'absence d'interactions.} En effet, le score suppose que les contributions des variables s'additionnent dans l'échelle des log-odds. Or la fraude se reconnaît souvent à des combinaisons de signaux (contrat récent + sinistre de nuit + absence de tiers identifié + véhicule en fin de vie par exemple). Chaque élément pris isolément est banal, c'est le croisement qui est suspect. Il faut créer les interactions à la main, donc les avoir devinées à l'avance.

\textbf{De plus, il existe une perte d'information par le \textit{binning}.} En effet, le découpage en classes lisse les non-linéarités mais détruit de la granularité et introduit des effets de seuil : deux dossiers quasi identiques de part et d'autre d'une frontière (ex. 31 jours vs 29 jours après souscription par exemple) reçoivent des points différents. Les bornes sont aussi une source de surapprentissage si elles sont optimisées sur les mêmes données que les poids.

Ce type de modèle est également \textbf{sensible aux biais présents dans les données historiques}, notamment les biais de sélection liés aux fraudes détectées (toutes les fraudes ne sont pas observées) et les biais d'investigation (certains dossiers étant plus contrôlés que d'autres). Ces biais peuvent affecter la cohérence du score et accentuer les défauts de classement.

Pire encore, il existe \textbf{une limite dynamique, liée au contexte \textit{adversarial}}, c'est-à-dire le fait que les fraudeurs essaient toujours de « faire mieux » pour ne pas être identifiés. Les fraudeurs s'adaptent. Une grille figée se dégrade plus vite qu'un modèle de risque classique, et si elle fuite en interne ou vers un intermédiaire, elle devient contournable point par point. Il faudrait alors un \textit{backtesting} rapproché et non pas un suivi de dérive ou une simple révision annuelle.
...
\begin{itemize}[label = $\bullet$]
    ...
    \item \textbf{Zone intermédiaire ($\approx$ 35~\% à 90~\%)} : on observe une tendance progressive du taux de fraude (avec cependant parfois des tranches de probabilité de fraude estimées supérieurs, ayant un taux de fraude avérée inférieur, \textbf{ce qui témoigne d'un défaut de linéarité du modèle}).
    À ce niveau, on commence à avoir un enrichissement significatif en fraude, tout en conservant un volume de dossiers encore réalisable manuellement pour des contrôles ciblés (revue documentaire, demandes complémentaires, etc.). 
    \item \textbf{Zone élevée ($>$ 90~\%)} : le taux de fraude devient nettement plus important avec plus d'un sinistre sur trois identifié comme frauduleux.
\end{itemize}
...
Le Gradient \textit{Boosting} s'impose comme le modèle retenu à retenir, car il offre le meilleur pouvoir discriminant parmi les trois approches testées : une AUC de 0\,933, un indice de Gini de 0\,867 et un lift de $\times$6,37 au premier décile. Il devance nettement le modèle basé sur un arbre de décision (AUC de 0\,845, indice de Gini de 0\,690, lift de $\times$4,83) et, plus légèrement, le modèle basé sur un \textit{scoring} en points (AUC de 0\,917, indice de Gini de 0\,835). Sa capacité à concentrer la fraude dans les premiers déciles en fait l'outil le plus efficace pour prioriser les dossiers à investiguer, ce qui correspond précisément à l'objectif opérationnel poursuivi.

Ce choix ne disqualifie pas pour autant le modèle basé sur un \textbf{\textit{scoring} en points, qui demeure un modèle remarquablement performant au regard de sa simplicité}. Avec une AUC de 0,917 et un indice de Gini de 0,835, il se situe très près du Gradient Boosting et loin devant l'arbre de décision, tout en conservant une transparence et une interprétabilité que les méthodes d'ensemble ne permettent pas. Il constitue donc une alternative pleinement crédible lorsque l'explicabilité prime sur le dernier point de performance, par exemple pour une présentation aux gestionnaires ou dans un contexte réglementaire exigeant. Le choix entre ces deux modèles relève ainsi d'un arbitrage entre performance pure (Gradient \textit{Boosting}) et interprétabilité (\text{scoring} en points), l'arbre de décision étant quant à lui écarté, dominé sur l'ensemble des critères.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/chapitre5/configuration.png}
