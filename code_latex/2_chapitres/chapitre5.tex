...
\begin{figure}[H]
    ...
    \caption{Hiérarchie de l'importance globale des variables dans le modèle de scoring en points}
    \label{fig:importance-variables-points}
\end{figure}

On remarque que l'importance des variables est ici remarquablement homogène : les dix variables retenues se répartissent le pouvoir explicatif de façon assez équilibrée, entre 8~\% et 13~\%, sans qu'aucune ne domine nettement les autres. Les contributions les plus fortes proviennent de l'historique des montants réclamés par l'assuré (13~\%), de la valeur marchande et de la durée de possession du véhicule (13~\% et 12~\%), suivies du nombre de fraudes passées, du ratio facture/marché et du nombre d'amendements à la déclaration (10~\% chacun).

Cette répartition équilibrée est cohérente avec la nature même du \textit{scoring} en points : reposant sur une agrégation additive de critères, le modèle répartit le risque sur un ensemble de signaux plutôt que de le concentrer sur quelques-uns. C'est une force car il ne dépend pas excessivement d'une seule variable, ce qui le rend robuste et stable (ce que confirment son PSI très faible et l'absence de surapprentissage), notamment si une des variables est manquante dans le dossier sinistre. 

\textit{ICI FAIRE LE COMMENTAIRE}
...
\begin{itemize}
    \item[$\bullet$] Le taux de non-fraudes bien classées (prédit non-fraude / réel non-fraude) : $5\,296 / 5\,312 = 99{,}70~\%$ ;
    \item[$\bullet$] Le taux de fraude détectées (prédit fraude / réel fraude) : $169 / 688 = 24{,}56~\%$.
    \item[$\bullet$] Le taux de sinistres signalés : $185 / 6\,000 = 3{,}42~\%$.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Matrice de confusion au seuil optimal du modèle de scoring en points (35~\%)}
...
\begin{itemize}
    \item[$\bullet$] Le taux de non-fraudes bien classées (prédit non-fraude / réel non-fraude) : $4\,566 / 5\,312 = 85{,}96~\%$ ;
    \item[$\bullet$] Le taux de fraude détectées (prédit fraude / réel fraude) : $548 / 688 = 79{,}65~\%$ ;
    \item[$\bullet$] Le taux de sinistres signalés : $1\,294 / 6\,000 = 21{,}57~\%$.
\end{itemize}
...
Malgré sa simplicité et son interprétabilité, le modèle de \textit{scoring} en points présente plusieurs limites dont il convient de tenir compte 
La première tient à sa construction même : le \textbf{n'est pas nécessairement strictement monotone au niveau de chaque variable}. En effet, le nombre de points attribué à une modalité ne suit pas toujours exactement l'ordre du risque sous-jacent. Ainsi, pour une variable comme le nombre de sinistres traités par le réparateur, la progression des points affectés ne reproduit pas rigoureusement celle du nombre de sinistres : deux modalités voisines peuvent recevoir une pondération qui ne respecte pas strictement leur hiérarchie de risque, comme on peut le constater sur la figure~\ref{fig:grille-points-reparateur} ci-dessous. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{images/chapitre5/points/grille_points_reparateur.png}
    \label{fig:grille-points-reparateur}
\end{figure}

Cette entorse à la monotonie découle de la nature additive et discrétisée du modèle, qui affecte les points par classe plutôt que par une fonction continue du risque.

Il faut toutefois distinguer cette limite, qui porte sur le barème variable par variable, de la qualité du classement global des dossiers. À l'échelle du score total, la relation avec la fraude reste, elle, remarquablement monotone : comme l'illustrait la figure \ref{fig:taux-fraude-points} présentée lors de l'analyse des résultats, le taux de fraude avérée croît régulièrement avec le score, passant de moins de 3~\% en deçà de 300 points à plus de 90~\% au-delà de 500 points, sans inversion entre tranches successives. Autrement dit, si le barème sous-jacent n'est pas parfaitement monotone au niveau de chaque critère, l'agrégation de l'ensemble des signaux produit néanmoins un score global qui ordonne correctement les dossiers selon leur risque de fraude (bien que cela aurait pu ne pas être le cas).

Par ailleurs, la transformation de variables continues en classes (via des tranches ou des modalités) entraîne une perte d'information et peut introduire des effets de seuil artificiels. Une légère variation d'un critère de part et d'autre d'une borne de classe peut modifier le score de façon discontinue, sans que le risque réel n'évolue de manière proportionnelle ; inversement, des différences significatives de profil au sein d'une même classe ne seront pas reflétées dans le score.

En outre, le \textit{scoring} en points ne fournit pas directement une probabilité calibrée de fraude, mais plutôt un indicateur ordinal de suspicion. L'interprétation du score en termes de probabilité nécessiterait une étape de calibration supplémentaire, ce qui peut compliquer la définition économique optimale des seuils de décision (arbitrage entre coût des faux positifs et gains liés à la fraude détectée).
...
Contrairement au modèle de \textit{scoring} en point, le profil est ici beaucoup plus contrasté : le ratio facture/marché du réparateur domine très largement, avec 34~\% de l'importance totale, soit à lui seul près d'un tiers du pouvoir explicatif du modèle. Viennent ensuite l'historique des montants réclamés (21~\%) et le nombre de fraudes passées de l'assuré (16~\%). À eux trois, ces critères concentrent plus de 70~\% de l'importance, les autres variables ne jouant qu'un rôle marginal (7~\% ou moins).

Cette concentration est caractéristique des arbres de décision : en construisant ses premières coupures sur les variables les plus discriminantes, l'arbre accorde un poids déterminant à un petit nombre de critères, ceux qui séparent le mieux les populations dès le sommet de l'arbre. Le ratio facture/marché s'impose ainsi comme le signal roi, ce qui rejoint directement l'analyse des corrélations où cette variable ressortait déjà comme l'une des plus liées à la fraude. Cette forte dépendance à quelques variables explique aussi la sensibilité de l'arbre au surapprentissage, déjà relevée par l'écart un peu plus marqué entre ses AUC d'entraînement et de validation : l'arbre s'ajuste finement à un nombre restreint de signaux, au risque de manquer de nuance sur les cas moins typiques.
...
\begin{itemize}
    \item[$\bullet$] Le taux de non-fraudes bien classées (prédit non-fraude / réel non-fraude) : $5\,167 / 5\,309 = 97{,}33~\%$ ;
    \item[$\bullet$] Le taux de fraude détectées (prédit fraude / réel fraude) : $269 / 688 = 39{,}10~\%$.
    \item[$\bullet$] Le taux de sinistres signalés : $411 / 5\,977 = 6{,}88~\%$.
\end{itemize}
...
\begin{itemize}
    \item[$\bullet$] Le taux de non-fraudes bien classées (prédit non-fraude / réel non-fraude) : $4\,211 / 5\,309 = 79{,}31~\%$ ;
    \item[$\bullet$] Le taux de fraude détectées (prédit fraude / réel fraude) : $512 / 688 = 74{,}41~\%$ ;
    \item[$\bullet$] Le taux de sinistres signalés : $411 / 5\,977 = 6{,}88~\%$.
\end{itemize}

L'efficacité opérationnelle de l'outil se mesure aussi à sa capacité à concentrer la fraude dans un volume restreint de dossiers à investiguer. Au seuil retenu, le modèle signale 6,88~\% des sinistres, parmi lesquels 31,80~\% sont réellement frauduleux, soit une densité de fraude environ 2,8 fois supérieure à celle d'un contrôle aléatoire. L'outil permet ainsi aux gestionnaires de concentrer leurs efforts sur un sous-ensemble à forte valeur, plutôt que de traiter l'ensemble du portefeuille.
...
Les paramètres du modèle de \textit{Gradient Boosting} créé sont les suivants :
...
Le Gradient Boosting présente un profil intermédiaire, plus équilibré que l'arbre mais toujours hiérarchisé. Le ratio facture/marché reste en tête (25~\%), suivi de l'historique des montants réclamés (16~\%) et du nombre de fraudes passées de l'assuré (12~\%), mais le pouvoir explicatif est ensuite mieux réparti sur une gamme plus large de variables (valeur du véhicule, amendements, durée de possession, âge du véhicule…), chacune contribuant de façon non négligeable.

Ce profil illustre bien ce qui fait la performance du Gradient Boosting : en combinant de nombreux arbres construits séquentiellement, le modèle conserve la hiérarchie des signaux forts (le ratio facture/marché demeure central) tout en exploitant l'information résiduelle portée par des variables secondaires que l'arbre unique négligeait. Il réalise ainsi un compromis entre la concentration de l'arbre et la dilution du \textit{scoring}, ce qui explique son meilleur pouvoir discriminant (AUC de 0,942). On retrouve par ailleurs les mêmes variables clés que dans les deux autres modèles (ratio facture/marché, historique de l'assuré, antécédents de fraude) ce qui témoigne de la robustesse de ces signaux quel que soit le modèle.

Le modèle \textit{Gradient Boosting} présente une très bonne séparation des deux classes (sinistres frauduleux / non frauduleux). La grande majorité des sinistres non frauduleux se concentre dans les tranches de faible probabilité (inférieures à 20~\%) tandis que la part de sinistres frauduleux (en rouge) croît fortement dans les tranches élevées, devenant prépondérante autour des scores supérieurs à 80~\%. 
...
Contrairement aux modèles plus simples, la montée du taux de fraude est ici beaucoup plus prononcée. Au-delà de 80~\%, on entre dans une zone où plus d'un sinistre sur deux est frauduleux, jusqu'à des taux proches de 100~\% sur la tranche la plus élevée. Cela signifie que le modèle concentre très efficacement les fraudes dans les segments à forte probabilité.
...
\begin{itemize}
    \item[$\bullet$] Le taux de non-fraudes bien classées (prédit non-fraude / réel non-fraude) : $5\,195 / 5\,312 = 97{,}80~\%$ ;
    \item[$\bullet$] Le taux de fraude détectées (prédit fraude / réel fraude) : $445 / 688 = 64{,}68~\%$.
    \item[$\bullet$] Le taux de sinistres signalés : $562 / 6\,000 = 9{,}37~\%$.
\end{itemize}
...
\begin{itemize}
    \item[$\bullet$] Le taux de non-fraudes bien classées (prédit non-fraude / réel non-fraude) : $4\,780 / 5\,312 = 89{,}98~\%$ ;
    \item[$\bullet$] Le taux de fraude détectées (prédit fraude / réel fraude) : $576 / 688 = 82{,}41~\%$ ;
...
Un seuil d'investigation fixé par défaut à $\pm$10~\%\footnote{Ces seuils d'investigation peuvent être modifiés par l'utilisateur s'il le souhaite.} du seuil optimal sera retenu, ainsi, si l'on considère par exemple un seuil optimal de 50~\%, trois zones d'investigations seront mises en place :
