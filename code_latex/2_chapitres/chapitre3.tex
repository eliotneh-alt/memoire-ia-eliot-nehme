...
\begin{itemize}
    \item[$\bullet$] vrai positif (TP) lorsque le modèle prédit une fraude et que le dossier est effectivement frauduleux ;
    \item[$\bullet$] vrai négatif (TN) lorsque le modèle prédit un dossier sain et que celui-ci l'est bien ;
    \item[$\bullet$] faux positif (FP) lorsqu'un dossier sain est incorrectement signalé comme frauduleux (génération d'un coût opérationnel inutile et une dégradation de l'expérience client) ;
    \item[$\bullet$] faux négatif (FN) lorsqu'un dossier frauduleux n'est pas détecté, constituant la perte financière directe pour la compagnie.
\end{itemize}
